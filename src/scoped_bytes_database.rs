...
use crate::{Scope, ScopedDbError, ScopedBytesCodec, utils::HeedRangeAdapter};
...
impl ScopedBytesDatabase {
    /// Creates a new ScopedBytesDatabase.
    ///
    ...
    ///
    /// This method creates a new ScopedBytesDatabase that uses the provided
    ...
    ///
    /// ```no_run
    /// # use scoped_heed::{ScopedBytesDatabase, ScopedDbError, Scope};
    /// # use heed::EnvOpenOptions;
    /// # fn main() -> Result<(), ScopedDbError> {
    /// # let env = unsafe { EnvOpenOptions::new().map_size(10*1024*1024).max_dbs(3).open("./db")? };
    /// # let mut wtxn = env.write_txn()?;
    /// # let db: ScopedBytesDatabase = ScopedBytesDatabase::new(&env, "test")?;
    /// # wtxn.commit()?;
    ...
    pub fn clear(
        ...
        match scope {
            ...
            Scope::Named { hash, .. } => {
                ...
                let _deleted_count = self.db_scoped.delete_range(txn, &range)?;
    ...
    pub fn range<'sbd_ref, 'txn_ref, 'bounds_ref, R>(
        ...
        Box<dyn Iterator<Item = Result<(&'txn_ref [u8], &'txn_ref [u8]), ScopedDbError>> + 'txn_ref>,
        ScopedDbError,
    >
    where
