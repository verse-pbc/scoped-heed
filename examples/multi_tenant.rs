...
fn main() -> Result<(), ScopedDbError> {
    ...
    // Populate data for tenant 1
    {
        ...
        users_db.put(&mut wtxn, Some(tenant1), &1, &User {
            id: 1,
            name: "Alice Johnson".to_string(),
            email: "alice@acme.com".to_string(),
        })?;
        
        users_db.put(&mut wtxn, Some(tenant1), &2, &User {
            id: 2,
        ...
        products_db.put(&mut wtxn, Some(tenant1), &101, &Product {
            id: 101,
    ...
    // Populate data for tenant 2 - using THE SAME keys but different scope
    {
        ...
        users_db.put(&mut wtxn, Some(tenant2), &1, &User {
            id: 1,
            name: "Charlie Davis".to_string(),
            email: "charlie@techstartup.io".to_string(),
        })?;
        
        users_db.put(&mut wtxn, Some(tenant2), &2, &User {
            id: 2,
        ...
        products_db.put(&mut wtxn, Some(tenant2), &101, &Product {
            id: 101,
    ...
    
    {
        ...
        println!("  Tenant 1 users: {:?}", users_db.get(&rtxn, Some(tenant1), &1)?);
        println!("  Tenant 2 users: {:?}", users_db.get(&rtxn, Some(tenant2), &1)?);
