...
fn main() -> Result<(), ScopedDbError> {
    ...
    let db = Arc::new(scoped_database_options(&env)
        .types::<String, i32>()
        .name("test_data")
        .create(&mut wtxn)?);
    wtxn.commit()?;
    ...
    for test_id in 1..=5 {
        ...
        let handle = thread::spawn(move || {
            ...
            db.put(&mut wtxn, Some(&test_scope), &"counter".to_string(), &0).unwrap();
            db.put(&mut wtxn, Some(&test_scope), &"status".to_string(), &1).unwrap();
            
            // Simulate test operations
            for i in 1..=10 {
                let counter_key = "counter".to_string();
                let current = db.get(&wtxn, Some(&test_scope), &counter_key)
                    .unwrap()
                    .unwrap_or(0);
                db.put(&mut wtxn, Some(&test_scope), &counter_key, &(current + i)).unwrap();
            }
            ...
            let final_counter = db.get(&rtxn, Some(&test_scope), &"counter".to_string())
                .unwrap()
                .unwrap();
            
            println!("Test {} completed. Final counter: {}", test_id, final_counter);
    ...
    // Verify all test scopes are cleaned up
    {
        ...
        for test_id in 1..=5 {
            let test_scope = format!("test_{}", test_id);
            let count: i32 = db.iter(&rtxn, Some(&test_scope))?
                .count()
                .try_into()
                .unwrap();
