...
use scoped_heed::{ScopedDatabase, ScopedBytesKeyDatabase, ScopedBytesDatabase};
...
fn benchmark_generic_database(c: &mut Criterion) {
    ...
    c.bench_function("generic_db_write", |b| {
        b.iter(|| {
            ...
            db.put(&mut wtxn, Some("scope1"), &key, &value).unwrap();
            wtxn.commit().unwrap();
        });
    });
    ...
    c.bench_function("generic_db_read", |b| {
        b.iter(|| {
            ...
            let _result = db.get(&rtxn, Some("scope1"), &key).unwrap();
            black_box(_result);
        });
    });
}
...
fn benchmark_bytes_database(c: &mut Criterion) {
    ...
    let db: ScopedBytesKeyDatabase<String> = ScopedBytesKeyDatabase::new(&env, "bench_bytes").unwrap();
...
fn benchmark_key_encoding_only(c: &mut Criterion) {
    use bincode;
    use scoped_heed::{ScopedKey, ScopedBytesCodec};
