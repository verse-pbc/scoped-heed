...
use scoped_heed::{Scope, ScopedDbError, scoped_database_options, GlobalScopeRegistry, ScopeRegistry};
...
fn test_scope_isolation_like_redis() -> Result<(), ScopedDbError> {
    ...
    // Create a shared scope registry
    let scope_registry = Arc::new(RwLock::new(ScopeRegistry::new()));
    
    // Create a database
    let mut wtxn = env.write_txn()?;
    let db = scoped_database_options(env, global_registry.clone())
        .with_scope_registry(scope_registry.clone())
        .types::<String, String>()
        .name("isolated_db")
        .create(&mut wtxn)?;
    wtxn.commit()?;
