...
use scoped_heed::{GlobalScopeRegistry, Scope, ScopedBytesDatabase, ScopedBytesKeyDatabase, ScopedDatabase, ScopedDbError};
use std::sync::Arc;
...
fn test_global_registry_basic() -> Result<(), ScopedDbError> {
    ...
    // Add data to scopes
    let mut wtxn = env.write_txn()?;
    db_users.put(&mut wtxn, &scope1, &"alice".to_string(), &"user data".to_string())?;
...
fn test_multiple_databases_sharing_registry() -> Result<(), ScopedDbError> {
    ...
    let mut wtxn = env.write_txn()?;
    db1.put(&mut wtxn, &tenant, &"key1".to_string(), &"value1".to_string())?;
    ...
    assert_eq!(empty_count_db1_after, 1, "DB1 should still find one empty scope");
    ...
    // Verify we have the shared_tenant in the registry using a scope to drop the transaction
    {
        ...
        assert_eq!(initial_scopes.len(), 2, "Registry should have two scopes (Default + shared_tenant)");
    ...
    assert_eq!(scopes_after_prune.len(), 1, "Registry should only have Default scope after pruning");
    assert!(scopes_after_prune.iter().all(|s| matches!(s, Scope::Default)), 
