...
fn test_iter_operations_with_scope_enum() -> Result<(), ScopedDbError> {
    ...
    // Insert data in multiple scopes
    {
        ...
        db.put(&mut wtxn, &Scope::Default, &"d".to_string(), &"default1".to_string())?;
        db.put(&mut wtxn, &Scope::Default, &"e".to_string(), &"default2".to_string())?;
        wtxn.commit()?;
    }
...
fn test_range_operations_with_scope_enum() -> Result<(), ScopedDbError> {
    ...
    // Insert ordered data
    {
        ...
        // Default scope
        db.put(&mut wtxn, &Scope::Default, &"a".to_string(), &"default_a".to_string())?;
        db.put(&mut wtxn, &Scope::Default, &"b".to_string(), &"default_b".to_string())?;
        db.put(&mut wtxn, &Scope::Default, &"c".to_string(), &"default_c".to_string())?;
