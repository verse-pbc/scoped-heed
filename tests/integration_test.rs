...
fn test_delete_operations() -> Result<(), ScopedDbError> {
    ...
    {
        let mut wtxn = env.write_txn()?;
        db.put(&mut wtxn, Some("scope1"), &"key1".to_string(), &"value1".to_string())?;
        db.put(&mut wtxn, Some("scope1"), &"key2".to_string(), &"value2".to_string())?;
        db.put(&mut wtxn, None, &"key1".to_string(), &"default_value".to_string())?;
        wtxn.commit()?;
    }
...
fn test_clear_operations() -> Result<(), ScopedDbError> {
    ...
    // Insert data in multiple scopes
    {
        ...
        db.put(&mut wtxn, Some("scope1"), &"key1".to_string(), &"value1".to_string())?;
        db.put(&mut wtxn, Some("scope1"), &"key2".to_string(), &"value2".to_string())?;
        db.put(&mut wtxn, Some("scope2"), &"key1".to_string(), &"value3".to_string())?;
        db.put(&mut wtxn, None, &"key1".to_string(), &"default_value".to_string())?;
        wtxn.commit()?;
    }
...
fn test_iter_operations() -> Result<(), ScopedDbError> {
    ...
    {
        let mut wtxn = env.write_txn()?;
        db.put(&mut wtxn, Some("scope1"), &"a".to_string(), &"value1".to_string())?;
        db.put(&mut wtxn, Some("scope1"), &"b".to_string(), &"value2".to_string())?;
        db.put(&mut wtxn, Some("scope1"), &"c".to_string(), &"value3".to_string())?;
        db.put(&mut wtxn, Some("scope2"), &"x".to_string(), &"value4".to_string())?;
        db.put(&mut wtxn, Some("scope2"), &"y".to_string(), &"value5".to_string())?;
...
fn test_range_operations() -> Result<(), ScopedDbError> {
    ...
    // Insert ordered data
    {
        ...
        // Scope1
        db.put(&mut wtxn, Some("scope1"), &"a".to_string(), &"scope1_a".to_string())?;
        db.put(&mut wtxn, Some("scope1"), &"b".to_string(), &"scope1_b".to_string())?;
        db.put(&mut wtxn, Some("scope1"), &"c".to_string(), &"scope1_c".to_string())?;
        db.put(&mut wtxn, Some("scope1"), &"d".to_string(), &"scope1_d".to_string())?;
    ...
    // Test exclusive range (a, d)
    {
        ...
        use std::ops::Bound;
        let range = (Bound::Excluded("a".to_string()), Bound::Excluded("d".to_string()));
